...
{
    public class Comp_Vehicle : ThingComp
    {
        public List<VehicleLoadingBill> bills = new List<VehicleLoadingBill>();
            ...
            if (!ResolvedITTab)
            {
                ...
                if (tabs != null && tabs.Count() > 0)
                    if (tabs.FirstOrDefault(x => x is ITab_Passengers) == null)
                        ...
                        {
                            LogUtil.LogError(string.Concat("Could not instantiate inspector tab of type ",
                            ...
                            for (var i = 0; i < group.handlers.Count; i++)
                            {
                                ...
                                for (var j = 0; j < pawn_needs.Count; j++)
                                {
                                    if (pawn_needs[j] is Need_Rest need_Rest)
                                    {
                                        ...
                                        Comp_VehicleUtility.TrySatisfyRestNeed(pawn, need_Rest, Pawn);
                                    }
                                    if (pawn_needs[j] is Need_Food need_Food)
                                    {
                                        pawn_needs[j].CurLevel -= Props.foodNeedRate;
                                        Comp_VehicleUtility.TrySatisfyFoodNeed(pawn, need_Food, Pawn);
                                    }
                                    if (pawn_needs[j] is Need_Chemical need_Chemical)
                                    {
                                        pawn_needs[j].CurLevel -= Props.foodNeedRate;
                                        Comp_VehicleUtility.TrySatisfyChemicalNeed(pawn, need_Chemical, Pawn);
                                    }
        ...
        public void GiveLoadJob(Thing thingToLoad, VehicleHandlerGroup group)
        {
            if (thingToLoad is Pawn pawn)
            {
                ...
                bills.Add(new VehicleLoadingBill(pawn, Pawn, group));
            }
        }
            ...
            if ((bills != null) & (bills.Count > 0))
            {
                ...
                if (bill != null)
                {
                    if (pawnToLoad.IsWorldPawn())
                        LogUtil.LogWarning("Called LoadPawn on world pawn");
                    ...
                    foreach (var group in handlers)
                    {
                        if (group.role != null)
                        {
                            ...
                            if (loadable || unloadable)
                            {
                                ...
                                //LogUtil.LogMessage(button.ToString());
                                yield return button;
                            }
                        }
                    }
