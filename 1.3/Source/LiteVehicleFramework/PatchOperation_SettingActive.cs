...
namespace LiteVehicles
{
    public class PatchOperation_SettingActive : PatchOperation
    {
        ...
        public override bool ApplyWorker(XmlDocument xml)
        {
            ...
            ModContentPack modContentPack = LiteVehiclesMod.mod.Content;
            for (int i = 0; i < settings.Count(); i++)
            {
                if (!LiteVehiclesMod.settings.IsValidSetting(settings[i]))
                {
                ...
                if (LiteVehiclesMod.settings.GetEnabledSettings.Contains(settings[i]))
                {
                    flag = true;
                    break;
                }
