...
namespace LiteVehicles
{
    public class ITab_Passengers : ITab
    {
        ...
        public float SpecificNeedsTabWidth
        {
            get
            {
        ...
        {
            DoRows(size, SelPawn?.GetComp<Comp_Vehicle>(), ref scrollPosition, ref scrollViewHeight, false,
                ref specificNeedsTabForPawn, doNeeds);
        }

        public static void DoRow(ref float curY, Rect viewRect, Rect scrollOutRect, Vector2 scrollPosition,
            Thing thing, Comp_Vehicle vehicle, ref Pawn specificNeedsTabForPawn, bool doNeeds)
        {
        ...
        public static void GetNeedsToDisplay(Pawn p, List<Need> outNeeds)
        {
            outNeeds.Clear();
        ...
        public static void DoRow(Rect rect, Thing thing, Comp_Vehicle vehicle, ref Pawn specificNeedsTabForPawn,
            bool doNeeds)
        {
            GUI.BeginGroup(rect);
...
#pragma warning restore IDE0019 // Use pattern matching
        ...
        public static bool AnyItemOrEmpty(List<Thing> things)
        {
            return things.Any(x => !(x is Pawn)) || !things.Any();
        }

        public static void DoRows(Vector2 size, Comp_Vehicle vehicle, ref Vector2 scrollPosition,
...
#pragma warning restore IDE0019 // Use pattern matching
        ...
        {
            base.UpdateSize();
            size = GetSize(SelPawn?.GetComp<Comp_Vehicle>()?.AllOccupants, PaneTopY, true);
            if (size.x + SpecificNeedsTabWidth > UI.screenWidth)
            {
                doNeeds = false;
                size = GetSize(SelPawn?.GetComp<Comp_Vehicle>().AllOccupants, PaneTopY, false);
            }
            else
            {
                doNeeds = true;
